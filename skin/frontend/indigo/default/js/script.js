	...
	if(jQuery('.color-block').length){
		function colorBlock(){
			jQuery('.color-block').each(function(){
				if(jQuery('body').hasClass('cms-about-magento-demo-store')){
					blockHeight = jQuery(this).find('.color-box-bg img').outerHeight();
				} else {
					blockHeight = jQuery(this).height() + parseFloat(jQuery(this).css('padding-top'));
				}
				console.log(jQuery(this).find('.color-box-bg img').height());
